...
| `settings-table`          | Adding or refactoring a reorderable, editable settings table using TableContainer, drag-and-drop, and either a single inline delete button (when delete is the only action) or OptionsMenuRootButton with Edit, Duplicate, Delete (when multiple actions are needed)                                                                             |
...
| `responsive-grid`         | Building a new component that arranges items in a responsive multi-column grid — dual auto-wrap/fixed-columns modes, ResizeObserver width measurement, two-breakpoint column switching, gap and alignment controls, and the debug width overlay                                                                                                  |
...
| `settings-dropdown`       | Adding single-choice select controls to editor settings                                                                                                                                                                                                                                                                                          |     | `settings-element-sections` | Organizing editor settings panels by UI element — GLOBAL first, then per-element sections with simplified labels. Use for large Style tabs or similar panels with many controls across multiple elements |     | `settings-section-headings` | **ALWAYS load this skill when creating or editing `editor.js`.** Use for every section heading and sub-section heading in any settings panel — correct CSS, class naming, and placement rules. |
...
| `triggers`                | Creating, naming, and exposing component triggers via getTriggers(). Use when a component fires events that should be available as right-click actions in the studio — or when triggers are defined but not exposing. Covers static and conditional patterns, firing via runTrigger(), naming conventions, and troubleshooting missing triggers. |
...
| `quiz-component`          | Building or reviewing a quiz component — stage structure (start → questions → end), per-stage typography inheritance via checkbox (inherit or override), optional prizes and lead forms, result determination patterns, and required upfront questions to ask the user before building                                                           |
| `optional-lead-form`      | Adding an optional lead form stage to a component (game, quiz, calculator) — placement (start vs results), system/custom field management, validation, skipping, and data mapping                                                                                                                                                                |
