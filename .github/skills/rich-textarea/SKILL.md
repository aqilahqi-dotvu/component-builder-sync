          ...
          <div key={link.id}>
            ...
            >
              ...
              <button
                ...
                type="button"
                onClick={() =>
                  onRemoveActionLink && onRemoveActionLink(link.id)
                }
              >
                Remove
              </button>
            ...
            <ActionSet
              ...
              onChange={(actionSet) =>
                onUpdateActionLink && onUpdateActionLink(link.id, actionSet)
              }
            />
          </div>
        ))}
...
```css
.your-component-caption strong {
  font-weight: 800 !important;
}
.your-component-caption em {
...
<RichTextEditor
  ...
  onRemoveActionLink={(linkId) => {
    updateState({
      heroCaptionLinkActions: (state.heroCaptionLinkActions || []).filter(
        (l) => l.id !== linkId,
      ),
    });
  }}
