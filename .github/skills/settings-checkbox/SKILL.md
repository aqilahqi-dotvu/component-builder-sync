...
- **Use `value` prop**, not `checked`
- **onChange receives the boolean value directly**, not an event object
- **`label` prop is required** and must be short (3–5 words)
- **Do not** use a `description` attribute — the component does not support it
...
// ❌ Treating onChange as an event handler
<Checkbox onChange={e => setState({ ...state, enabled: e.currentTarget.checked })} ... />
...
<SettingItem>
  ...
  <Checkbox value={state.enabled} onChange={enabled => setState({ ...state, enabled })} />
</SettingItem>
```
