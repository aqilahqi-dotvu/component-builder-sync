...
description: Rules and patterns for implementing shadow settings in editor.js, common.js, and live.js
---
...
- **Do** gate all shadow detail controls (`shadowOnHover`, `shadowColor`, `shadowOffsetX`, `shadowOffsetY`, `shadowBlur`, `shadowSpread`) behind a `hasShadow` conditional.
- **Do** use `rgba(...)` as the default `shadowColor` in `common.js`. It accepts both hex strings and `rgba(...)` strings — the `resolveShadowColor` helper in `live.js` normalises hex to `rgba` at render time.
- **Do** clamp numeric inputs with safe `min`/`max` bounds: offset ±100, blur 0–120, spread ±50.
- **Do** compute `boxShadow`, `restingBoxShadow`, and `hoverBoxShadow` in `live.js` using the `resolveShadowColor` helper and the `s()` scaler so all values scale with the component.
...
- **Do not** show shadow detail controls when `hasShadow` is `false`. Keep the editor clean.
- **Do not** add a separate opacity slider. Opacity is encoded inside `shadowColor` as the alpha channel of the `rgba` value.
...
```jsx
<Section>
  <div className="settings-section-heading">Shadow</div>
  <SettingItem>
    <Checkbox
...
Compute the shadow values inside the component, before the CSS template string:
...
// legacyShadowOpacity is only used when shadowColor is a plain hex without alpha
...
// ❌ Hex color interpolated directly — invalid CSS
box-shadow: 0px 12px 32px 0px ${state.shadowColor};

// ❌ Separate opacity slider — opacity belongs in shadowColor alpha channel
<NumberInput value={state.shadowOpacity} min={0} max={1} step={0.01} onChange={...} />

// ❌ Numeric values not scaled — shadow will not respond to component scaling
box-shadow: ${shadowOffsetX}px ${shadowOffsetY}px ${shadowBlur}px ${shadowSpread}px ...;
```
