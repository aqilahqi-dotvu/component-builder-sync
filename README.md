...
| `add-live-drag.prompt.md`         | `/add-live-drag`         | Add drag-to-reposition behavior to elements rendered directly inside a live.js component — pointer events, position state, and constraints. |
...
| `responsive-grid/`            | `/responsive-grid`            | Complete responsive grid starter — dual-mode layout (auto-wrap flex or fixed CSS grid), ResizeObserver measurement, two-breakpoint column switching, gap and alignment controls, and the debug width overlay.                                                                                                                                                                                         |
...
| `quiz-component/`             | `/quiz-component`             | Build or review a quiz component — stage structure (start → questions → end), per-stage typography inheritance via checkbox (inherit or override), optional prizes and lead forms, result determination patterns, and required upfront questions to ask the user before building.                                                                                                                     |
| `optional-lead-form/`         | `/optional-lead-form`         | Add an optional lead form stage to a component (game, quiz, calculator) — placement (start vs results), system/custom field management, validation, skipping, and data mapping.                                                                                                                                                                                                                       |
...
| `settings-buttons/`           | `/settings-buttons`           | Pattern for Shape-based button styling — six shape presets (Square, Rounded, Pill, and Outlined variants), state fields, CSS variable derivation, and a getButtonShapeStyle() helper. Use when a component has configurable output buttons that need visual shape control.                                                                                                                            |
| `settings-dotvu-buttons/`     | `/settings-dotvu-buttons`     | Rules for using Button and IconButton controls in editor.js — mandatory style prop, primary vs secondary variants, icon-only buttons with IconButton, available icons from @icons, children vs text prop, and disabled/selected states.                                                                                                                                                               |
| `settings-range-slider/`      | `/settings-range-slider`      | Rules for single-thumb and dual-thumb range sliders in editor.js — styled native HTML inputs, buildTicks helper, accent color, and Label pairing. Use when a setting needs a visual slider or when a min/max pair should be combined into one dual-thumb control.                                                                                                                                     |
| `settings-image-positioning/` | `/settings-image-positioning` | Rules and patterns for adding a 3×3 image-position picker — POSITION_GRID constant, per-item objectPosition state field, orange-highlighted button grid in the Drawer, and CSS object-position application in live.js. Use when a component renders images with object-fit:cover and needs a focal-point control.                                                                                     |
| `drag-position-picker/`       | `/drag-position-picker`       | Drag-and-drop viewport position picker for placing multiple floating elements at arbitrary x%/y% positions — dark mini-viewport preview in the Drawer, draggable numbered handles with image thumbnails, optional reference list of sibling labels, and per-item `images: [{ url, pos }]` state. Use when a component needs per-item, freely positioned floating elements rather than a fixed layout. |
| `settings-drawer/`            | `/settings-drawer`            | Rules and patterns for Drawer and DrawerSection in editor.js — isOpen prop, always-mounted pattern, drawer width matching settings panel width, section headings inside portaled drawers, and splitting large drawers into multiple DrawerSection blocks. Use when adding or editing an item-edit Drawer in a component.                                                                              |
...
| `carousel-component.md` | A boilerplate for carousel/slider components with navigation, pagination, and per-slide content.                                                     |
